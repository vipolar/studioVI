...
sudo apt-get install -y fail2ban curl wget git git-lfs python3 python3-tk python3-pip python3-venv libgl1 libglib2.0-0 dotnet-sdk-8.0 google-perftools debian-keyring debian-archive-keyring apt-transport-https
...
sudo mkdir -p /var/www/html && sudo cp index.html /var/www/html/index.html
cat .resources/Caddyfile | sudo tee /etc/caddy/Caddyfile > /dev/null
